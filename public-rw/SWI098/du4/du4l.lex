...
{IDENT}										*l = line; lv->id_ci_ = ctx->tab->ls_id().add(parseIdent(yytext)); return DUTOK_IDENTIFIER;
{DIGIT}*									*l = line; lv->int_ci_ = ctx->tab->ls_int().add(parseInt(yytext)); return DUTOK_UINT;
[0-9]+(\.[0-9]+)?([Ee][+-]?[0-9]+)?			*l = line; lv->real_ci_ = ctx->tab->ls_real().add(parseReal(yytext)); return DUTOK_REAL;
{DIGIT}*{IDENT}								*l = line; lv->int_ci_ = ctx->tab->ls_int().add(parseInt(yytext)); return DUTOK_UINT; /*broken int*/
[0-9]+(\.[0-9]+)?([Ee][+-]?[0-9]+)?{IDENT}	*l = line; lv->real_ci_ = ctx->tab->ls_real().add(parseReal(yytext)); return DUTOK_REAL;
...
<STR><<EOF>>							*l = line; error(DUERR_EOFINSTRCHR, line); lv->str_ci_ = ctx->tab->ls_str().add(strval); BEGIN(INITIAL); strval = ""; return DUTOK_STRING;
<STR>\n									*l = line; error(DUERR_EOLINSTRCHR, line); lv->str_ci_ = ctx->tab->ls_str().add(strval); ++line; BEGIN(INITIAL); strval = ""; return DUTOK_STRING;
<STR>''									*l = line; strval += "'";
<STR>'									*l = line; lv->str_ci_ = ctx->tab->ls_str().add(strval); BEGIN(INITIAL); strval = ""; return DUTOK_STRING;
